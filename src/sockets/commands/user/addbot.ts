...
export const addbot: Command = {
  command: 'addbot',
  help: '/addbot [count]: Host-only. Add one or more SimpleBot seats before the game starts.',
  run: async (args, socket) => {
    ...
    const result = room.addStandaloneBots(parseInt(args[1] || '1', 10), socket.request.user.username);
    if (result.success) {
